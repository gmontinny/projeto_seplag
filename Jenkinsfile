...
def deployToPortainer(String imageName, String imageTag) {
    // Detectar porta do EXPOSE no Dockerfile
    def appPort = '8080'
    if (fileExists('Dockerfile')) {
        def expose = sh(script: "grep -i '^EXPOSE' Dockerfile | head -1 | grep -oE '[0-9]+'", returnStdout: true).trim()
        if (expose) { appPort = expose }
    }

    // Ler .env do projeto e montar bloco environment para o compose
    def envBlock = ''
    if (fileExists('.env')) {
