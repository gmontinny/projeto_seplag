...
def deployToPortainer(String imageName, String imageTag) {
    // Detectar porta: EXPOSE direto, ARG default, ou .env
    def appPort = '8080'
    if (fileExists('Dockerfile')) {
        // Tentar EXPOSE com numero direto
        def expose = sh(script: "grep -i '^EXPOSE' Dockerfile | head -1 | grep -oE '[0-9]+' || true", returnStdout: true).trim()
        if (expose) {
            appPort = expose
        } else {
            // EXPOSE usa variavel - buscar ARG default
            def argPort = sh(script: "grep -i '^ARG.*PORT' Dockerfile | head -1 | grep -oE '[0-9]+' || true", returnStdout: true).trim()
            if (argPort) { appPort = argPort }
        }
    }
    // .env pode sobrescrever
    if (fileExists('.env')) {
        def envPort = sh(script: "grep -i '^APP_PORT=' .env | cut -d= -f2 || true", returnStdout: true).trim()
        if (envPort) { appPort = envPort }
    }

    // Montar bloco environment do .env
    def envBlock = ''
    if (fileExists('.env')) {
        def envLines = readFile('.env').trim().split('\n')
    ...
    withCredentials([usernamePassword(credentialsId: 'portainer-credentials', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
        ...
        def stackContent = "services:\\n  app:\\n    image: ${imageName}:${imageTag}\\n    restart: unless-stopped\\n    ports:\\n      - ${appPort}:${appPort}${envBlock}"

        sh """
            curl -s -X POST 'https://portainer:9443/api/stacks/create/standalone/string?endpointId=1' \
              ...
              }' || echo "Stack ja existe, atualizando imagem..."
